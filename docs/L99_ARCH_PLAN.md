...
| S28 | **Dual-mode requirement (owner, 2026-07-03):** the tool must support BOTH story forms as first-class — (1) NARRATED reels (VO carries the story; today's strength) and (2) CINEMATIC dialogue (characters speak in-scene, story moves through performance). Verified today: script→plan already handles mode 2 (dialogue→speaker-tagged shots, correct character focus per line — tested on the Yamraj script); the gaps are all in the PERFORMANCE layer | 🚧 track started — **step 0 SHIPPED** (form detect→declare: per-shot dialogue/narration/silent tags + story verdict badge + voice warning; deterministic, override-able) · **T4 SHIPPED** (owner's own commit 06e9b5b — voice picker + voice_map; functionally verified). **Location anchoring SHIPPED** (2026-07-14, S30 Phase 1 — derive_locations/set_location + invariant clause + empty plates, docs/S30_ADOPTION_PLAN.md) · **SFX seam SHIPPED** (agents/sfx.py + models.json "mmaudio"; the Final-Cut mix stem is the ticketed remainder). Remaining: D5 → T7 → SFX mix stem. Original:  T4 per-character voice UI (small — dropped from the executed order, first up), location/scene anchoring (M — the 'character sheet for places'), D5 two-character identity refs (M), T7 selective lip-sync on dialogue close-ups (L, kill-criterion stands), SFX/atmosphere layer (M, new). Narrated mode stays the default; cinematic is a per-story choice, one engine (no fork). |
| S32 | **SaaS pricing model (owner, 2026-07-17):** credits/seats like galleri5, with an admin portal tracking who burned what — "can we give the same for much less?" | 📋 RESEARCHED + PROPOSED → **docs/PRICING_PLAN.md** (not decided). galleri5's bundle decoded: flat **₹1/credit**, ~3–20× markup, no volume discount. Proposal: same unit, three deviations — break their linearity, credits never expire, **denominate in finished reels not generations** (on raw generations Magnific gives unlimited Kling video at ₹2,250 — unwinnable). Ladder ₹999 / ₹2,499 / ₹12,499 (all under the **RBI ₹15k** silent-auto-renew ceiling — their ₹40k tier is not). Free tier = **unlimited mood boards + 1 reel** (~₹150 COGS) not a credit count. Stack: Razorpay + own Postgres double-entry ledger + hold-then-settle (we already have that shape in `reserve_spend`). **Blocking: no tenancy** — one namespace, every run visible to every login. Verdict on raising: **no** — pilot first. |
| S31 | **One creator mode (owner, 2026-07-17):** kill the Story/Brand/Studio front doors — Canvas is the product; entry becomes a prompt box + scope chips (Runway-shaped), not a 25-control form; reuse Story's posting kit | ✅ DECIDED → **docs/CANVAS_ENTRY_PLAN.md**. Studio DELETE (redundant), Story = the DEFAULT, Brand = the `Product Ad` scope (keeps the ad-claims gate + CTA + disclosure). Parity audit: canvas ~85–90%, AHEAD on 9 capabilities, no pipeline fork to unwind. **Blocking pre-flight:** (0) canvas AI-disclosure label never fires (`subject_name:""` hardcoded → reels labeled "no real person depicted"; fix the label, KEEP the no-consent-gate call) · (1) canvas can't ingest photos from a browser — THE parity blocker · (2) posting kit → `agents/` · (3) surface export/provenance/perf · (4) brand gate must fire via canvas · (5) `/api/canvas/plan` unauthenticated. |
| S29 | **Podcast/presenter format (owner, 2026-07-08):** script + owner's photo → 16:9 podcast episode — presenter (owner's animated face, lip-sync) speaks to camera, edit cuts to beat-matched B-roll; canvas scope selector grows (podcast, education) with per-scope controls | 🚧 Phase 1 partly shipped → **docs/PRESENTER_PLAN.md** — **1a scope registry SHIPPED** (2026-07-18, `shot_planner._SCOPE_SYSTEM_PROMPTS`, general/commerce only) · **1b's found bug fixed** (canvas orientation control already worked end-to-end for 9:16/16:9 — a stale planning-doc assumption; the real bug was `_run_inner` silently mapping Square → landscape dims, now routed through the existing `_orient_wh`). Golden-face spike (Phase 0, gates all presenter feature code) **not run**; fal account still balance-locked (blocks 2 of 3 spike legs — see S30). Extends S28's form axis with `presenter`; T7's preview-gate discipline inherited, its 3-beat cap replaced by spend cap. |
| S30 | **galleri5 live-run teardown (owner-driven, 2026-07-14):** Yamraj script run end-to-end on their Agentic Canvas + their JS bundle mined. Findings: asset-first plate→composite architecture (empty location plates w/ negative space, flat-lit single-frontal character refs); house style-token injected over the operator's brief (director-intent bug, and leaky at video stage); auto SFX (MMAudioV2 video→audio) + Lyria music + beat-synced push-in made the clip feel finished; per-asset review states; canvas = rented tldraw. **Validates the remaining S28 order** (location anchoring first, SFX layer real) + the continuity P1/P2 beat-sync bet. Red-team yield: passthrough-as-feature is copyable — the durable moat half is HOB archive + consent relationships → asset library (GAP #6) under-prioritized at P2. Their single-frontal identity lock walks into our already-diagnosed S19/S20 drift. Feeds S17 (their side now captured). | 🚧 executing → **docs/S30_ADOPTION_PLAN.md** — **Phase 1 SHIPPED + verified** (location anchoring: derive/set/plate + 🏞 panel; 9/9 offline checks, live LLM smoke derived the same 2 locations galleri5 did on the Yamraj beats) · **Phase 2 seam SHIPPED** (agents/sfx.py + "mmaudio" model + sfx pricing; degradation-safe; Final-Cut mix stem ticketed — S1 lesson) · **Phase 3 SHIPPED** (per-asset review states: metadata-only `review_status` + board chips + Inspector select; sticky bar closed as already-covered by `.cv2-bottom` + inspector-actions) · found live: fal balance EXHAUSTED (403 account-locked — blocks ALL fal generation until top-up). Decision record: COMPETITOR_GALLERI5_TEARDOWN.md §10 |
