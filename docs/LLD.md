...
| `/api/canvas/<run_id>/render` | POST (operator) | Render the board into a reel by dispatching `_execute_pipeline` (the proven engine); **reuses the Key Frames render dir** so stills are cached. Sets `render_phase="full"`. Same governance gates as `/run`. Progress on `/progress/<render_id>`; per-shot clips via `clip_ready`. (`/rendered` reconciles paid stage chips by `render_phase` so Key Frames-done ≠ Video-done) |
