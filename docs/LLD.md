...
| `/api/canvas/<run_id>/character` | POST (operator) | Attach a **real reference photo + consent** to one character (`canvas_run.set_character(char_id, ref_path, consent)`). Links every shot whose `speaker_id==char_id` to that face (sets `character_ref_path` + keeps `ai_portrait`) so identity is consistent across the reel. Path validated `_path_allowed`; photo uploaded via `/upload-photo`. The moat for **multi-person** stories: real identity, consent-gated. No ref → that character stays generic |
